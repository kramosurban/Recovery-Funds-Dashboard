...
tribble(
  ~"Jurisdiction", ~"Total Appropriations in Dashboard (billions)",  ~"Percent of Appropriations in Dashboard that are Un-Expired (billions)", 
  "City of Chicago", chicago_allocation_b, pct_chicago_unexp_b,
  "Cook County", cook_county_allocation_b, pct_cook_county_unexp_b,
  "Illinois State", il_state_allocation_b, pct_il_unexp_b
) %>% DT::datatable(
  caption = htmltools::tags$caption(
    style= 'caption-side: bottom; text-align: left',
    'Un-expired funds are not past the date at which funds are no longer available for expenditure. Expiration dates vary by program.'),
  options = list(dom = 't'), 
  rownames = FALSE) %>%
  formatCurrency(c(2), 
                 currency = "$",  
